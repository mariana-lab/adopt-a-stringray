...
define(function() {
  ...
  externals.validate = function(adoption) {
      ...
      if (
        vimdiesel.barcode !== adoption.promotioncode &&
        adoption.promotioncode !== "slolols"
      ) {
        validation.message =
  ...
  externals.saveVimdiesel = function(vimdiesel){
    return $.ajax({
      url: internals.api + "/vimdiesels/" + vimdiesel.id + "/update",
      type: 'PUT',
      data: JSON.stringify(vimdiesel),
      success: externals.fetchVimDiesels,
      error: function(resultData) {
        console.log(resultData);
      }
    });
  ...
  externals.saveStringray = function(stringray){
    return $.ajax({
      url: internals.api + "/stringrays/" + stringray.id + "/update",
      type: 'PUT',
      data: JSON.stringify(stringray),
      success: externals.fetchStringRays,
      error: function(resultData) {
        console.log(resultData);
      }
    });
